...
Implementation of [[Simplex Noise]] with [[Fractal Noise]] in different languages.
...
export function getFractal(input_x: number, input_y: number, octaves: number) {
  let total = 0.0;
  let frequency = 1.0;
  let amplitude = 1.0;
  let maxValue = 0.0;
  ...
  for (let i = 0; i < octaves; i++) {
    total += get(input_x * frequency, input_y * frequency) * amplitude;
    maxValue += amplitude;
    amplitude *= 0.5;
    frequency *= 2.0;
  }
